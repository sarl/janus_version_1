 ...
 */
public class LispExecutionContextTest extends TestCase {

	private static final String ADDITION_SCRIPT_NAME = "addition.lisp"; //$NON-NLS-1$
	private static final String INTERPRETED_ADDITION_SCRIPT_NAME = "interpretedaddition.lisp"; //$NON-NLS-1$
	private static final String HELLO_WORLD_SCRIPT_NAME = "HelloWorld.lisp"; //$NON-NLS-1$
	private static final String MY_PRINT_SCRIPT_NAME = "Print.lisp"; //$NON-NLS-1$
	private static final String METHOD_SCRIPT_NAME = "Method.lisp"; //$NON-NLS-1$
	
	private LispExecutionContext interpreter;
	...
	protected void setUp() throws Exception {
		...
		this.additionScript = Resources.getResource(LispExecutionContext.class, ADDITION_SCRIPT_NAME);
		this.scriptDirectory = FileSystem.convertURLToFile(this.additionScript).getParentFile();
		this.interpreter = new LispExecutionContext(new ScriptEngineManager());
	...
	public void testMakeFunctionCall() throws Exception {
		...
		assertEquals("(myFunc 1.5d0 2.0d0)", //$NON-NLS-1$
				this.interpreter.makeFunctionCall("myFunc", 1.5d, new BigDecimal(2))); //$NON-NLS-1$
		
		assertEquals("(myFunc NIL 3)",  //$NON-NLS-1$
	...
	public void testRunCommandString() throws Exception {
		Object v = this.interpreter.runCommand("(null NIL)"); //$NON-NLS-1$
		assertNotNull(v);
		this.listener.assertFalse();

		v = this.interpreter.runCommand("(+ 5.7d0 6)"); //$NON-NLS-1$
		assertEquals(11.7d, v);
		this.listener.assertFalse();
	...
	public void testRunScriptString() throws Exception {
		...
		this.interpreter.setStandardOutput(output);
		v = this.interpreter.runScript(HELLO_WORLD_SCRIPT_NAME);
		assertEquals("hello world", v); //$NON-NLS-1$
		assertEquals("\n\"hello world\" ", output.toString()); //$NON-NLS-1$
		this.listener.assertFalse();
	...
	public void testRunFunctionStringStringObjectArray() throws Exception {
		...
		v = this.interpreter.runFunction(
				...
				"myprint", //$NON-NLS-1$
				"ab\"c"); //$NON-NLS-1$
		assertEquals("ab\"c", v); //$NON-NLS-1$
		assertEquals("\n\"ab\\\"c\" ", output.toString()); //$NON-NLS-1$
		this.listener.assertFalse();
	}
	...
	public void testMethodInvocation() throws Exception {
		...
		assertEquals("\n\"my message was printed\" ", output.toString()); //$NON-NLS-1$
		this.listener.assertFalse();
	}
