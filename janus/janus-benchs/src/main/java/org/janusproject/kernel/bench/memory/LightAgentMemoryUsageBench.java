 ...
 * @since 0.5
 */
public class LightAgentMemoryUsageBench extends CsvBench<MemoryUsageBenchRun> {

	/**
	 * @param directory
	 * @throws IOException
	 */
	public LightAgentMemoryUsageBench(File directory) throws IOException {
		super(directory);
	}
	...
	public void setUpUnitaryBench(MemoryUsageBenchRun run) throws Exception {
		...
		for(int i=0; i<getCurrentRun().getNumberOfAgents(); ++i) {
			kernel.launchLightAgent(new IddleAgent());
		}
