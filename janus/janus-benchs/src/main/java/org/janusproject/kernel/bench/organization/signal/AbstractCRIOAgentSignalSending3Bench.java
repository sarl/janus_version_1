...
/**
 * n/2 listeners are registered in the role.
 * n/2 listeners are registered in the player.
 * 
 ...
 * @since 0.5
 */
public abstract class AbstractCRIOAgentSignalSending3Bench extends AbstractRoleOperationPerSecondCsvBench<OperationAgentNumberBenchRun> {

	/**
	 * @param directory
	 * @throws IOException
	 */
	public AbstractCRIOAgentSignalSending3Bench(File directory) throws IOException {
		super(directory,
				"Listeners", //$NON-NLS-1$
	...
	public static class EmitterAgent extends BenchMarkedAgent {
		...
		public Status activate(Object... params) {
			int nb = ((Integer)params[0]).intValue();
			int nbInRole = nb - nb/2;
			nb -= nbInRole;
			SignalListener listener = new IddleSignalListener();
			for(int i=0; i<nb; ++i) {
				addSignalListener(listener);
			}
			return super.activate(new Object[] {nbInRole});
		}
