...
/**
 * (n-2) agents are playing the inactive version of the benchmarked role (GetRole or PeekRole)
 * 1 agent is playing the EmitterRole
 * 1 agent is playing the active version of the benchmarked role (GetRole or PeekRole)
 * 
 ...
 * @since 0.5
 */
public abstract class AbstractCRIOAgentMessageReceiving2Bench extends AbstractRoleOperationPerSecondCsvBench<OperationAgentNumberBenchRun> {

	/**
	 * @param directory
	 * @throws IOException
	 */
	public AbstractCRIOAgentMessageReceiving2Bench(File directory) throws IOException {
		super(directory,
				"Players", //$NON-NLS-1$
	...
	protected org.janusproject.kernel.bench.api.AbstractRoleOperationPerSecondCsvBench.BenchMarkedAgent launchAgents(
		...
		if (run.getName().startsWith("PeekMessage")) { //$NON-NLS-1$
			for(int i=0; i<run.getNumberOfAgents()-2; ++i) {
				kernel.submitLightAgent(new BenchMarkedAgent(
						launchFlag, BenchOrganization.class,
						PeekRole.class,
						false,
						false));
			}
			receiver = new BenchMarkedAgent(launchFlag, BenchOrganization.class, PeekRole.class, false, true);
			...
			receiver = new BenchMarkedAgent(launchFlag, BenchOrganization.class, GetRole.class, false, true);
	...
	public static class GetRole extends BenchMarkedRole {
		...
		public Status activate(Object... params) {
			this.isActive = ((Boolean)params[0]).booleanValue();
			return null;
		}
