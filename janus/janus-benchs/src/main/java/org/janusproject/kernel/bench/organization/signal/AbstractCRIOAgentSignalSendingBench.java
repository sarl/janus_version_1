...
/**
 * n listeners are registered in the role.
 * 0 listener is registered in the player.
 * 
 ...
 * @since 0.5
 */
public abstract class AbstractCRIOAgentSignalSendingBench extends AbstractRoleOperationPerSecondCsvBench<OperationAgentNumberBenchRun> {

	/**
	 * @param directory
	 * @throws IOException
	 */
	public AbstractCRIOAgentSignalSendingBench(File directory) throws IOException {
		super(directory,
				"Listeners", //$NON-NLS-1$
	...
	protected final org.janusproject.kernel.bench.api.AbstractRoleOperationPerSecondCsvBench.BenchMarkedAgent launchAgents(
		...
		BenchMarkedAgent emitter = new BenchMarkedAgent(
				launchFlag,
				BenchOrganization.class,
				EmitterRole.class,
				true,
				run.getNumberOfAgents()); // stands for getNumberOfSignals
