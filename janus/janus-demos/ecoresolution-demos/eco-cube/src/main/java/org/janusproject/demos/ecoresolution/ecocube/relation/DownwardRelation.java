...
/** Relationship "Master is above Slave"
 * 
 ...
 */
public class DownwardRelation extends AbstractEcoRelation<DownwardRelation,UpwardRelation> {
	
	private static final long serialVersionUID = -6330741449636600442L;
	 ...
	 * @param table
	 */
	public DownwardRelation(EcoIdentity master, EcoIdentity slave, EcoIdentity table) {
		super(master, slave);
		this.table = table;
	}
	...
	public boolean equals(EcoRelation relation) {
		return CubeRelationUtil.isDownwardEqual(getMaster(), getSlave(), relation);
	}
	 ...
	 */
	@Override
	public UpwardRelation invert() {
		return new UpwardRelation(getSlave(), getMaster(), this.table);
	}
	...
	public EcoRelation toPattern(EcoIdentity participantToRemove) {
		...
		if (m.equals(participantToRemove)) {
			return new DownwardRelation(AnyIdentity.SINGLETON, s, this.table);
		}
		if (s.equals(participantToRemove)) {
			return new DownwardRelation(m, AnyIdentity.SINGLETON, this.table);
		}
		return this;
	}
	...
	public String toString() {
		return Locale.getString(DownwardRelation.class, "TEXT", getMaster(), getSlave()); //$NON-NLS-1$
	}
