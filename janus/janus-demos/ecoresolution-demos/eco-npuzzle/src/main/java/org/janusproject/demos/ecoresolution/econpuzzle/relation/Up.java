 ...
 */
public class Up extends AbstractEcoRelation<Up,Down> {

	private static final long serialVersionUID = -3304474965693329267L;

	/**
	 * @param master
	 * @param slave
	 */
	public Up(EcoIdentity master, EcoIdentity slave) {
		super(master, slave);
	}
	...
	public EcoRelation toPattern(EcoIdentity participantToRemove) {
		...
		if (m.equals(participantToRemove)) {
			return new Up(AnyIdentity.SINGLETON, s);
		}
		if (s.equals(participantToRemove)) {
			return new Up(m, AnyIdentity.SINGLETON);
		}
		return this;
	}
	...
	public boolean equals(EcoRelation relation) {
		return NPuzzleRelationUtil.isUpEqual(getMaster(), getSlave(), relation);
	}

	@Override
	public Down invert() {
		return new Down(getSlave(), getMaster());
	}
