...
/**
 * This class provides a tuned formatter for log records from kernels.
 * <p>
 * The format of each line is:<br>
 * <code>&lt;DATE&gt; Agent[&lt;NAME&gt;,&lt;ID&gt;] &lt;LEVEL&gt;: &lt;MESSAGE&gt;</code><br>
 * 
 ...
 */
public class JanusKernelFormatter
extends AbstractJanusFormatter {

	private final Address kernel;
	
	/**
	 * @param timeManager is the kernel time manager.
	 * @param kernel is the kernel for which the log messages may be formatted.
	 */
	public JanusKernelFormatter(KernelTimeManager timeManager, Address kernel) {
		super(timeManager);
		assert(kernel!=null);
		this.kernel = kernel;
	}
	...
	public String getFormatterOwner() {
		return this.kernel.getUUID().toString();
	}
	...
	public String getLocalizedMessage(LogRecord record, String message) {
		...
		return Locale.getString(JanusKernelFormatter.class,
				LoggerUtil.isShortLogMessageEnable()
				? "SHORT_MESSAGE" //$NON-NLS-1$
				: "MESSAGE", //$NON-NLS-1$
				formatDate(),
				this.kernel.getUUID().toString(),
				action,
				message);
	}
