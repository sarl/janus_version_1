 ...
 */
package org.janusproject.kernel.network.agent;
...
import org.janusproject.kernel.crio.core.MembershipService;
...
import org.janusproject.kernel.network.api.NetworkAdapter;
import org.janusproject.kernel.network.api.NetworkListener;
 ...
 */
public class NetworkingKernelAgent extends KernelAgent implements RepositoryChangeListener, NetworkListener {
	 ...
	 */
	NetworkingKernelAgent(AgentActivator activator, Boolean commitSuicide, EventListener startUpListener, String applicationName, NetworkAdapter networkAdapter, BundleContext context) {
		super(activator, commitSuicide, null, startUpListener, networkAdapter, applicationName);
		getAddress().setName(Locale.getString(NetworkingKernelAgent.class, "NAME")); //$NON-NLS-1$
	 ...
	 */
	@Override
	public AgentAddress receiveOrganizationalDistantMessage(GroupAddress group, Class<? extends Role> receiverRole, Message message, boolean isBroadcast) {
		if (isBroadcast) {
	...
	private class GroupCleaner implements Runnable {
		...
		public void run() {
			NetworkingKernelAgent.this.removeInactivePersistentGroups(
