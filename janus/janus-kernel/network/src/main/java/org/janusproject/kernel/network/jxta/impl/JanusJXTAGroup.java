...
import org.janusproject.kernel.network.jxta.JXTANetworkHandler;
import org.janusproject.kernel.network.utils.OSGiHelper;
...
abstract class JanusJXTAGroup extends JxtaGroup {
	 ...
	 * @throws IOException
	 */
	public AgentAddress sendMessage(Message message) throws IOException {
		for (int i = 0; i < MAX_SEND_MESSAGE_RETRY; i++) {
			PipeAdvertisement pAdv = findCandidateKernel();
			if (pAdv != null) {
				AgentAddress receiver = sendMessage(message, false, pAdv);
				if (receiver != null) {
					return receiver;
				}
			}
	...
	private AgentAddress sendMessage(Message message, boolean isBroadcast, PipeAdvertisement pAdv) throws IOException {
	 ...
	 */
	protected abstract AgentAddress processIncomingMessage(Message janusMessage, boolean isBroadcast);
	...
	private class IncommingConnectionHandler implements PipeMsgListener {
		...
		public void pipeMsgEvent(PipeMsgEvent event) {
			...
			try {
				...
				AgentAddress receiver = JanusJXTAGroup.this.processIncomingMessage(janusMessage, isBroadcast);
	...
	private class OutgoingConnectionHandler implements PipeMsgListener {
		...
		AgentAddress responseAddress = null;
