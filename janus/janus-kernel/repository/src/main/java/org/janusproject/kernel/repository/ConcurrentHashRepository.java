...
/** A {@link ConcurrentHashMap}-based implementation of a Repository.
 *  
 ...
 */
public class ConcurrentHashRepository<ID,DATA>
extends AbstractRepository<ID,DATA> {

	private final ConcurrentHashMap<ID,DATA> content; 
	...
	public DATA remove(ID key) {
		...
		if (data!=null) fireRemoveRepositoryChange(key, data);
		return data;
	}
