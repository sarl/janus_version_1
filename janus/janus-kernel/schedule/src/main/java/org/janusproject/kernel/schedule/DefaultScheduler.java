 ...
 */
public class DefaultScheduler<A extends Activator<?>>
extends AbstractScheduler<A> {

	/** 
	 * @param type is the type of supported activable objects.
	 */
	public DefaultScheduler(Class<A> type) {
		super(type);
	}
	 ...
	 */
	public DefaultScheduler(Class<A> type, Collection<? extends A> scheduledObjects) {
	...
	protected Status executeInit(Iterator<? extends A> objects, Object... parameters) {
		...
		while (objects.hasNext()) {
			try {
				obj = objects.next();
				obj.sync();
				ms.addStatus(obj.activate(parameters));
			}
